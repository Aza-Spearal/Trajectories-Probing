...
removed_classifiers = [
    ...
    "MultiOutputClassifier", #MultiOutputClassifier(LogisticRegression(max_iter=300000))
    "MultinomialNB", 
    "OneVsOneClassifier", #
    "OneVsRestClassifier", #
...
#CLASSIFIERS.append(("LGBMClassifier", lightgbm.LGBMClassifier(force_col_wise=True)))
...
CLASSIFIERS.append(("ClassifierChain", multioutput.ClassifierChain(LogisticRegression(max_iter=300000))))
    ...
    def fit(self, X_train, X_test, y_train, y_test):
        ...
        for name, model in tqdm(self.classifiers):
            ...
            try:
                ...
                elif name == 'RadiusNeighborsClassifier':
                    pipe = Pipeline(steps=[("preprocessor", preprocessor), ("classifier", model(outlier_label='most_frequent'))])
                    pipe.fit(X_train, y_train)
                elif name == 'NuSVC':
                    for nu in range(10):
                        try:
        ...
        #scores = scores.sort_values(['Accuracy', 'F1 Score', 'Time Taken'], ascending=[False, False, True]).set_index("Model")
        scores = scores.sort_values(['F1 Score', 'Accuracy', 'Time Taken'], ascending=[False, False, True]).set_index("Model")
        #scores = scores.sort_values(['F1 Score'], ascending=[False]).set_index("Model")
