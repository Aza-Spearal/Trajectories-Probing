...
class LabeledTokenizer:
    ...
    def tokenize(self, data, max_char, indice, function):
        ...
        for i, split in enumerate(tqdm(data)):

            for chunck in split['split_text']:

                text = chunck['text'][:int(max_char)]
                label = chunck['category']

                if len(text)>0 and label != 'other':

                        try:
                            with torch.no_grad():
                                if function == 'residual':
                                    residual = self.model.get_residual_stream(text) #[64, n_token, dmodel]
                                else:
                                    residual = self.model.get_activations(text) #[64, n_token, dmodel]
                                mean = torch.mean(residual.to(device), dim=1, keepdim=True).cpu() #[64,1,288]
...
data_files=['humanEdited_code.json', 'humanEdited_explanatory.json', 'humanEdited_instructional.json', 'humanEdited_narrative.json', 'humanEdited_speech.json']
data = list()
for fil in data_files:
    with open(fil, 'r') as infile:
        data.extend(json.load(infile))

labels = ['instructional', 'explanatory', 'narrative', 'speech', 'code']
